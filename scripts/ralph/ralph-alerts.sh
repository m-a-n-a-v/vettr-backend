#!/bin/bash
# Ralph Alerts - Automated agent loop
# Iterates through alerts-prd.json stories using Claude
# Targets: Backend, iOS, Web, Android, Ops Dashboard repos
...
OPS_DIR="/Users/manav/Space/code/vettr-ops-dashboard"
PRD="$BACKEND_DIR/scripts/ralph/alerts-prd.json"
INSTRUCTIONS="$BACKEND_DIR/scripts/ralph/CLAUDE-alerts.md"
PROGRESS="$BACKEND_DIR/scripts/ralph/alerts-progress.txt"

MAX_ITERATIONS=16

echo "Starting Ralph (Alerts) - Max iterations: $MAX_ITERATIONS"
...
if [ ! -f "$PROGRESS" ]; then
  echo "# Ralph Progress Log - VETTR Alerts" > "$PROGRESS"
  echo "Started: $(date)" >> "$PROGRESS"
  echo "Project: VETTR Alerts - Complete E2E Implementation" >> "$PROGRESS"
  echo "---" >> "$PROGRESS"
fi

# Copy instruction files into each repo so Claude can read them
sync_files() {
    for DIR in "$BACKEND_DIR" "$IOS_DIR" "$WEB_DIR" "$ANDROID_DIR" "$OPS_DIR"; do
...
for i in $(seq 1 $MAX_ITERATIONS); do
    echo ""
    echo "==============================================================="
    echo "  Ralph Alerts Iteration $i of $MAX_ITERATIONS"
    echo "==============================================================="
...
")
    ...
    OUTPUT=$(cd "$WORK_DIR" && claude --model claude-sonnet-4-5 --dangerously-skip-permissions --print \
        "You are Ralph, an autonomous coding agent implementing VETTR Alerts - the complete E2E alerts system.
...
If ALL stories have passes=true after your update, output exactly: <promise>COMPLETE</promise>" \
    ...
    if echo "$OUTPUT" | grep -qF "<promise>COMPLETE</promise>"; then
        echo ""
        echo "Ralph (Alerts) completed all tasks!"
